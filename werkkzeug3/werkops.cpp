...
#define TEXTSIZEX ((app->DefaultTexSize >> 0) & 0xff)
#define TEXTSIZEY ((app->DefaultTexSize >> 8) & 0xff)
...
#define COL_ALL   5
#define COL_HIDE  6
#define COL_WZT   0x80      // werkzeug for textures
...
void Edit_Misc_EventTime(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
}
...
void Edit_Misc_Shaker(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags ("Mode"         , 0,0,"fadein|fadeout|smooth|always:*4around cam|around center:*5sin|perlin");
...
void Edit_Misc_SplineScale(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFloat ("Scale"        , 0,3, 1.0f,-256, 256,0.001f);
  app->ParaWin->EditFlags ("cancel rotation", 3,0,"*0|x:*1|y:*2|z");
}
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags ("Width"        , 0,TEXTSIZEX,"1|2|4|8|16|32|64|128|256|512|1024|2048:*7|lock");
  app->ParaWin->EditFlags ("Height"       , 1,TEXTSIZEY,"1|2|4|8|16|32|64|128|256|512|1024|2048");
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags ("Mode"         , 0,0,"add|sub|mul|diff|alpha|brightness|hardlight|over|addsmooth|min|max");
}
...
void Edit_Bitmap_Format(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  if(!app->TextureMode)
    app->ParaWin->EditFlags ("Format"       , 0,1,"|A8R8G8B8|A8 (ps30 only)|||Q8W8V8U8 (normal map)|||DXT1|DXT5||I8|A8I8 (ps30 only)");
  else
    app->ParaWin->EditFlags ("Format"       , 0,1,"|A8R8G8B8||||Q8W8V8U8 (normal map)|||DXT1|DXT5");
  ...
  app->ParaWin->EditFlags ("DXT Quality"  , 2,0,"*6fast (no dither)|medium (dither)");
}

void Edit_Bitmap_RenderTarget(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags ("Width"        , 0,TEXTSIZEX,"1|2|4|8|16|32|64|128|256|512|1024|2048");
  app->ParaWin->EditFlags ("Height"       , 1,TEXTSIZEY,"1|2|4|8|16|32|64|128|256|512|1024|2048");
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditPos2D ("Center"       , 0,2,0.50f,-4.0f, 4.0f,0.001f);
  app->ParaWin->EditPos2D ("Radius"       , 2,2,0.25f, 0.0f, 4.0f,0.001f);
  app->ParaWin->EditPos2D ("Size"         , 4,2,0.00f, 0.0f, 4.0f,0.001f);
  ...
  app->ParaWin->EditFlags ("Power Mode"   ,10,1,"normal|alternative");
  app->ParaWin->EditFlags ("Gradient shape",10,1,"*1ellipse|rectangle");
}
...
void Edit_Bitmap_GlowRect(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditRGBA  ("Color"        , 6,0xffffffff);
  app->ParaWin->EditFloat ("Blend"        , 7,1, 1.0f, 0.0f, 1.0f,0.01f);
  app->ParaWin->EditFloat ("Power (hardness)" , 8,1, 0.5f, 0.0f,16.0f,0.001f);
  app->ParaWin->EditFlags ("Wrap"         , 9,0,"off|on");
  app->ParaWin->EditFlags ("Power Mode"   ,10,0,"normal|alternative");
  app->ParaWin->EditFlags ("Gradient shape",10,0,"*1ellipse|rectangle");
}
...
void Edit_Bitmap_Blur(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  if(!app->TextureMode)
    app->ParaWin->EditFlags ("Passes"         , 0,0x12,"0|1|2|3|4|5:*4old paras|new paras");
  else
    app->ParaWin->EditFlags ("Passes"         , 0,0x12,"0|1|2|3|4");
...
void Edit_Bitmap_Rotate(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditAngles("Angle"        , 0,1,0.00f,-4.0f, 4.0f,0.001f);
  app->ParaWin->EditFloat ("Scale"        , 1,2,1.00f,-16.0f,16.0f,0.001f);
  app->ParaWin->EditPos2D ("Offset"       , 3,2,0.50f,-4.0f, 4.0f,0.001f);
  app->ParaWin->EditFlags ("Border clamp" , 5,0,"off|x|y|both");
  app->ParaWin->EditFlags ("Filter"       , 5,0,"*2bilinear|nearest");
}
...
void Edit_Bitmap_Twirl(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditCycle ("Border clamp" , 6,0,"off|x|y|both");
}
...
void Edit_Bitmap_Distort(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditCycle ("Clamp at Border"       , 1,0,"off|x|y|both");
}
...
void Edit_Bitmap_Normals(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFlags ("Mode"         , 1,1,"2d|3d|tangent 2d|tangent 3d:*2normal|extra sharp");
}
...
void Edit_Bitmap_Light(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditPos2D ("Pos"          , 1,3,0.50f,-16.0f,16.0f,0.02f);
...
void Edit_Bitmap_Bump(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFloat ("Power (hardness)" ,12,1,16.0f,  0.0f,  256,0.02f);
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditPos2D ("Position"     , 0,2,0.000f,-4.0f, 4.0f ,0.001f);
  app->ParaWin->EditFloat ("Size"         , 2,1,0.125f,-16.0f,16.0f,0.001f);
  app->ParaWin->EditFloat ("Leading"      , 8,1,1.0f,-16.0f,16.0f,0.001f);
  app->ParaWin->EditFloat ("Stretch"      , 3,1,0.00f,0.0f,16.0f,0.001f);
  ...
  app->ParaWin->EditFlags ("Include font outline" , 5,0,"*7no|yes");
  if(!app->TextureMode)
  {
...
void Edit_Bitmap_Cell(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditInt   ("Max"          , 5,1,  255,    2,  255,0.25f);
...
void Edit_Bitmap_Bricks(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditInt   ("Single Width Stones"   ,10,1,   80,    0,  255,8);
  app->ParaWin->EditFlags ("Flags"        ,11,6,"*2|no adjacent singles:*3|joints color");
...
void Edit_Bitmap_Gradient(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditAngles("Angle"        , 5,1,0.00f,-4.0f, 4.0f,0.001f);
  app->ParaWin->EditFloat ("Width of Gradient"       , 6,1,1.00f,-16.0f,16.0f,0.001f);
  app->ParaWin->EditCycle ("Mode"         , 7,0,"linear|gaussian|sine");
}
...
void Edit_Bitmap_Sharpen(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFloat ("Radius"       , 1,2,0.005f, 0.0f, 4.0f,0.0001f);
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("Filename"     , 0,"Images:png");
  app->ParaWin->AddBox(CMD_PARA_RELOAD ,0,0,"reload");
}

void Edit_Bitmap_Import2(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("Filename"     , 0,"Images:png");
  app->ParaWin->EditInt   ("Quality"      , 0,1, 82,0,100,0.25f);
  app->ParaWin->AddBox(CMD_PARA_RELOAD ,0,0,"reload");
}
...
void Edit_Bitmap_Unwrap(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFlags ("Border clamp" , 0,0,"*4off|x|y|both");
}
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditCycle ("Width"        , 0,TEXTSIZEX,"1|2|4|8|16|32|64|128|256|512|1024|2048");
  app->ParaWin->EditCycle ("Height"       , 1,TEXTSIZEY,"1|2|4|8|16|32|64|128|256|512|1024|2048");
}
...
void Edit_Bitmap_Export(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditString("Texture name" , 0);
  app->ParaWin->EditFlags ("Alpha channel in .PNG", 0,1, "no|yes");
}

void Edit_Bitmap_Paste(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFloat ("Pre-scale"    , 0,2,1.0f,-64.0f,64.0f,1.0f/1024.0f);
  app->ParaWin->EditPos2D ("Position"     , 2,2,0.5f,-64.0f,64.0f,1.0f/1024.0f);
  app->ParaWin->EditPos2D ("Size"         , 4,2,1.0f,-64.0f,64.0f,1.0f/1024.0f);
  app->ParaWin->EditAngles("Angle"        , 6,1,0.0f,-4.0f,4.0f,1.0f/1024.0f);
  app->ParaWin->EditFlags ("Mode"         , 7,0,"copy|over:*7bilinear|nearest");
}

void Edit_Bitmap_Geloet(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditCycle ("Width"          , 0,TEXTSIZEX,"1|2|4|8|16|32|64|128|256|512|1024|2048");
  app->ParaWin->EditCycle ("Height"         , 1,TEXTSIZEY,"1|2|4|8|16|32|64|128|256|512|1024|2048");
  app->ParaWin->EditFlags ("Flags"          , 2,0,"|wrap");
  app->ParaWin->EditGroup ("Graph layout");
  app->ParaWin->EditInt   ("Points/iter"    , 3, 1,25,2,255,0.25f);
  app->ParaWin->EditInt   ("Edges/iter"     , 4, 1,50,1,255,0.25f);
  app->ParaWin->EditInt   ("Iterations"     , 5, 1,10,1,255,0.25f);
  app->ParaWin->EditInt   ("Max path len"   , 6, 1,48,1,255,0.25f);
  app->ParaWin->EditInt   ("Random seed"    , 7, 1,0,0,255,0.25f);
  app->ParaWin->EditGroup ("Painting");
  app->ParaWin->EditFloat ("Line diameter"  , 8, 1,3.0f,0,12,0.01f);
  app->ParaWin->EditFloat ("Contact radius" , 9, 1,3.0f,0,12,0.01f);
  app->ParaWin->EditFloat ("Contact width"  ,10, 1,1.5f,0,12,0.01f);
}

void Edit_Bitmap_VectorPath(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("Filename"     , 0,"Paths:path");
  app->ParaWin->EditRGBA    ("Color"        , 0,0xff000000);
  app->ParaWin->EditFlags   ("Fill rule"    , 1,0,"nonzero winding|even-odd");
  app->ParaWin->AddBox(CMD_PARA_RELOAD ,0,0,"reload");
}
...
void Edit_Effect_Water(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditLink  ("Material"     , 0);
  app->ParaWin->EditFloat ("Scale Grid" , 0, 1, 1.0f,1.0f,256,0.001f);
  app->ParaWin->EditFloat ("Scale Pos"  , 1, 1, 1.0f,1.0f,256,0.001f);
  app->ParaWin->EditFloat ("Scale UV"   , 2, 1, 1.0f,1.0f,256,0.001f);
  app->ParaWin->EditFloat ("Speed"      , 3, 1, 1.0f,0.1f,256,0.01f);
  app->ParaWin->EditFloat ("Scale Amp"  , 4, 1, 1.0f,0.1f,256,0.001f);
  app->ParaWin->EditInt   ("Grid"       , 5, 1, 7,5,8,1);
...
void Edit_Effect_PartEmitter(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditInt   ("Multiple Slots",17,1,1,1,     15,0.25f);
...
//  app->ParaWin->EditFlags ("Mode"         , 0,0,":*4|one burst:*8|reset");
  ...
  app->ParaWin->EditGroup ("Object emitter only");
  app->ParaWin->EditFloat ("*Fade"     ,18,1, 0.0f,  -64,   64,0.001f);
  app->ParaWin->EditFloat ("*Metamorph",19,1, 0.0f,  -64,   64,0.001f);
}
...
void Edit_Effect_PartSystem(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditLink  ("Material"     , 0);
  app->ParaWin->EditInt   ("Particle Slot", 0,1,0,0,     15,0.25f);
  app->ParaWin->EditInt   ("Max Count"    , 1,1,1024,1,0x40000,16);
  ...
  app->ParaWin->EditFloat ("*Gravity"      ,10,1, 0.0f, -256,  256,0.001f);
...
void Edit_Effect_Tourque(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFlags ("Flags"           , 3,0,"free|chain|cyclic:*4tri|quad:*5emit always|one burst:*6|cubes");
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("File name",0,"Images:png");
  app->ParaWin->AddBox(CMD_PARA_RELOAD,0,0,"reload");
...
void Edit_Effect_Geloet2D(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditCycle ("Grid width"      , 1,5,"1|2|4|8|16|32|64|128|256|512|1024");
  app->ParaWin->EditCycle ("Grid height"     , 2,5,"1|2|4|8|16|32|64|128|256|512|1024");
  app->ParaWin->EditFlags ("!Flags"          , 3,0,"|wrap:*1|greetings:*2|pimp my precalczeit");
  if(*op->Op.GetAnimPtrU(3) & 2)
    app->ParaWin->EditString("Greetings text"  , 0);

  app->ParaWin->EditLink  ("Line Material"   , 0);
  app->ParaWin->EditLink  ("Point Material"  , 1);
  app->ParaWin->EditGroup ("Graph layout");
  app->ParaWin->EditInt   ("Points/iter"    , 4, 1,25,2,255,0.25f);
  app->ParaWin->EditInt   ("Edges/iter"     , 5, 1,50,1,255,0.25f);
  app->ParaWin->EditInt   ("Iterations"     , 6, 1,10,1,255,0.25f);
  app->ParaWin->EditInt   ("Max path len"   , 7, 1,48,1,255,0.25f);
  app->ParaWin->EditInt   ("Random seed"    , 8, 1,123,0,255,0.25f);
  app->ParaWin->EditGroup ("Graph painting");
  app->ParaWin->EditFloat ("*Animate"        , 0,1,0.0f,0.0f,1.0f,0.001f);
  app->ParaWin->EditFloat ("*Line width"     , 9,1,0.18f,0.0f,2.0f,0.001f);
  app->ParaWin->EditFloat ("*Point width"    ,10,1,0.36f,0.0f,2.0f,0.001f);
}

void Edit_Effect_GeloetMesh(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditLink  ("Line Material"   , 0);
  app->ParaWin->EditLink  ("Point Material"  , 1);
  app->ParaWin->EditGroup ("Graph layout");
  app->ParaWin->EditInt   ("Points/iter"    , 1, 1,25,2,255,0.25f);
  app->ParaWin->EditInt   ("Edges/iter"     , 2, 1,50,1,255,0.25f);
  app->ParaWin->EditInt   ("Iterations"     , 3, 1,10,1,255,0.25f);
  app->ParaWin->EditInt   ("Max path len"   , 4, 1,48,1,255,0.25f);
  app->ParaWin->EditInt   ("Random seed"    , 5, 1,123,0,255,0.25f);
  app->ParaWin->EditGroup ("Graph painting");
  app->ParaWin->EditFloat ("*Animate"        , 0,1,0.0f,0.0f,1.0f,0.001f);
  app->ParaWin->EditFloat ("*Line width"     , 6,1,0.18f,0.0f,2.0f,0.001f);
  app->ParaWin->EditFloat ("*Point width"    , 7,1,0.36f,0.0f,2.0f,0.001f);
}

void Edit_Effect_Glare(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditInt   ("Pass"             , 0,1, 1,0,31,0.125f);
  app->ParaWin->EditFlags ("Downsample"       , 1,0,"orig->512|orig->256|orig->512->256:*2Gauss|Linear");
  app->ParaWin->EditRGBA  ("*Treshold Color"   , 2,0xff808080);
  app->ParaWin->EditFloat ("*Gray<->Color"     , 9,1,0.0f,-16,16,0.01f);
  app->ParaWin->EditRGBA  ("*Glare Color"      , 3,0xff808080);
  app->ParaWin->EditRGBA  ("*Screen Color"     , 4,0xffffffff);
  app->ParaWin->EditFloat ("*Add<->AddSmooth"  ,10,1,0.0f,-16,16,0.01f);
  app->ParaWin->EditFloat ("*Pass 1 Radius"    , 5,1,0.01f,0,1,0.0001f);
  app->ParaWin->EditFloat ("*Pass 1 Amplify"   , 7,1,1.0f,0,16,0.01f);
  app->ParaWin->EditFloat ("*Pass 2 Radius"    , 6,1,0,0,1,0.0001f);
  app->ParaWin->EditFloat ("*Pass 2 Amplify"   , 8,1,1.0f,0,16,0.01f);
}

void Edit_Effect_ColorCorrection(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditInt   ("Pass"             , 0,1, 1,0,31,0.125f);
  app->ParaWin->EditGroup ("Dark/Bright");
  app->ParaWin->EditFloat ("*Dark Range"       , 1,1, 0.0f,-16.0f,16.0f,0.01f);
  app->ParaWin->EditFloat ("*Bright Range"     , 2,1, 1.0f,-16.0f,16.0f,0.01f);
  app->ParaWin->EditFlags ("Flags",7,0,"|Show Range");
  app->ParaWin->EditRGBA  ("*Dark Color"      ,4, 0xff000000);
  app->ParaWin->EditRGBA  ("*Dark Gray"       ,9, 0xffffffff);
  app->ParaWin->EditRGBA  ("*Bright Color"    ,3, 0xffffffff);
  app->ParaWin->EditRGBA  ("*Bright Gray"     ,8, 0xff000000);
  app->ParaWin->EditGroup ("Final Color");
  app->ParaWin->EditRGBA  ("*Subtract"        ,6, 0xff000000);
  app->ParaWin->EditRGBA  ("*Color"           ,5, 0xffffffff);
  app->ParaWin->EditFloat ("*Brightness"      ,10,1,1,0,16.0f,0.01f);

}

void Edit_Effect_GridStuff(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditLink  ("Material"       , 0);
  app->ParaWin->EditInt   ("Size"             , 0,2, 1,0,255,0.125f);
  app->ParaWin->EditRGBA  ("*Color 1"         ,2, 0xff000000);
  app->ParaWin->EditRGBA  ("*Color 2"         ,3, 0xffffffff);
}

void Edit_Effect_Cubes(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditGroup ("Geometry");
  app->ParaWin->EditInt   ("Pass"             ,11,1,0,0,255,0.125f);
  app->ParaWin->EditFlags ("Rendering flags"  ,12,0x0d,"|Shadow map:*1|Reflection:*2|Ground plane:*3|Main cubes");
  app->ParaWin->EditFlags (""                 ,12,0x0d,"*4|Main NoShadow");
  app->ParaWin->EditFloat ("Up vector"        , 0,3,0.0f,-1024.0f,1024.0f,0.01f);
  app->ParaWin->EditFloat ("Groundplane Y"    , 9,1,0.0f,-1024.0f,1024.0f,0.01f);
  app->ParaWin->EditFloat ("Groundplane Size" ,10,1,100.0f,-1024.0f,1024.0f,0.01f);
  app->ParaWin->EditGroup ("Light");
  app->ParaWin->EditRGBA  ("Light Color"      , 3,0xffe0e0e0);
  app->ParaWin->EditRGBA  ("Dark Color"       ,15,0xff181818);
  app->ParaWin->EditRGBA  ("Reflection light" ,13,0xffe0e0e0);
  app->ParaWin->EditRGBA  ("Reflection dark"  ,16,0xff181818);
  app->ParaWin->EditFloat ("Direction"        , 4,3,0.0f,-1024.0f,1024.0f,0.01f);
  app->ParaWin->EditFloat ("Distance"         , 7,1,50.0f,-1024.0f,1024.0f,0.01f);
  app->ParaWin->EditFloat ("Depth range"      , 8,1,150.0f,0.0f,1024.0f,0.01f);
  app->ParaWin->EditFloat ("Shadow opacity"   ,14,1,1.0f,0.0f,1.0f,0.001f);

  // groundplane blah etc später

  if(op->SetDefaults)
  {
    *op->Op.GetEditPtrF(1) = 1.0f; // up: default up
    *op->Op.GetEditPtrF(6) = 1.0f; // direction: default +z
  }
}

void Edit_Effect_BounceWorm(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);

  app->ParaWin->EditInt   ("seed"         , 0,1,5,0,255);
  app->ParaWin->EditFloat ("*grav"         , 1,1,1,0.0f,16.0f,0.01f);
  app->ParaWin->EditFloat ("*damp"         , 2,1,0.9993f,0.99f,1.0,0.00001f);
  app->ParaWin->EditFloat ("*float"        , 3,1,1,0.0f,16.0f,0.01f);
  app->ParaWin->EditFloat ("*size"         , 4,1,1,0.0f,16.0f,0.001f);
  app->ParaWin->EditInt   ("timeslice"     , 5,1,10,5,15);
  app->ParaWin->EditInt   ("skipcubes"     , 6,1,0,0,255);
}

void Edit_Effect_Wirbeln(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFloat ("speed"        , 0,1,0.01f,-1.0f,1.0f,0.001f);
  app->ParaWin->EditFloat ("speed delta"  , 1,1,0.5f,0.0f,16.0f,0.01f);
  app->ParaWin->EditFloat ("size"         , 2,1,0.125f,0.0f,16.0f,0.001f);
  app->ParaWin->EditFloat ("*radius"       , 3,1,6,0.0f,16.0f,0.001f);
  app->ParaWin->EditFloat ("*width"        , 4,1,0.5f,0.0f,16.0f,0.001f);
  app->ParaWin->EditFloat ("*height"       , 5,1,0.5f,0.0f,16.0f,0.001f);

  app->ParaWin->EditInt   ("wobble1"      , 6,1,11,0,64);
  app->ParaWin->EditInt   ("wobble2"      , 7,1,5,0,64);
  app->ParaWin->EditFloat ("*strength"     , 8,1,2,0.0f,256,0.01f);
  app->ParaWin->EditFloat ("*yscale"       , 9,1,3,0.0f,256,0.01f);
  app->ParaWin->EditFloat ("*yadd"         ,10,1,2,0.0f,256,0.01f);
  app->ParaWin->EditInt   ("count"        ,11,1,0x8000,0,0x10000,0x100);
}

void Edit_Effect_XSI2Cube(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("File name"      , 0,".xsi");
  app->ParaWin->AddBox(CMD_PARA_RELOAD,0,0,"reload");
  app->ParaWin->EditFlags ("Flags"        , 0,0,"|Normalize");
}

void Edit_Effect_CubeMorpher(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
}

void Edit_Misc_ImageList(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditInt   ("Resolution"   , 0,2, 512,1,2048,0.25f);
}

void Edit_Effect_ImageJukebox(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditLink  ("Image list"   , 0);
  app->ParaWin->EditInt   ("Render pass"  , 0,1,0,0,31,0.125f);
  app->ParaWin->EditInt   ("Image index"  , 1,1,0,0,255,0.125f);
  app->ParaWin->EditFloat ("Whiten"       , 2,1,0.0f,0.0f,1.0f,0.001f);
  app->ParaWin->EditFloat ("Amplify"      , 3,1,1.0f,0.0f,64.0f,0.001f);
  app->ParaWin->EditFloat ("Fade"         , 4,1,1.0f,0.0f,1.0f,0.001f);
}
...
void Edit_Material_Material(WerkkzeugApp *app,WerkOp *op)
{
  ...
  if(*op->Op.GetEditPtrU(48) & 0x1e)
  {
    ...
    app->ParaWin->EditFlags (""             ,48,0x00020b,"*11|instances");
    if(multiFlags & 0x10)
  ...
  else
  {
    ...
    app->ParaWin->EditFlags ("Special"      , 1,0x000000,"*4no envi|sphere envi|reflect envi|use normal:*8no proj|model proj|world proj|eye proj");
...
void Edit_Material_Material20(WerkkzeugApp *app,WerkOp *op)
{
  ...
  if(!app->TextureMode)
  {
    app->ParaWin->EditFlags ("!Flags"           , 0, 0x30,"*0|specular:*1|paralax:*2|smooth light:*3|vertex color");
    app->ParaWin->EditFlags ("!"                , 0, 0x30,"*4|shadows:*5|renorm bump:*6|x2 intensity:*7|specularity map");
    app->ParaWin->EditFlags ("!"                , 0, 0x30,"*8|alpha burn:*9|col correct main");
  }
  else
  {
    app->ParaWin->EditFlags ("!Flags"           , 0, 0x30,"*0|specular:*6|x2 intensity:*7|specularity map");
  }
  app->ParaWin->EditRGBA  ("Diffuse"          , 1, 0xffffffff);
  app->ParaWin->EditRGBA  ("Specular"         , 2, 0xffffffff);
  app->ParaWin->EditFloat ("Specular power (hardness)"   , 3, 1,16.0f,1.0f,1024.0f,0.01f);
  if(*op->Op.GetEditPtrU(0) & 0x002) // paralax?
    app->ParaWin->EditFloat ("Paralax strength" , 4, 1,0.01f,0.0f,2.0f,0.001f);
  else if(*op->Op.GetEditPtrU(0) & 0x100) // alpha burn?
    app->ParaWin->EditFloat ("Burn factor"      , 4, 1,0.00f,0.0f,1.0f,0.001f);
  ...
  if(*op->Op.GetEditPtrU(0) & 0x200) // color correct
  {
    app->ParaWin->EditGroup("Main color correct");
    app->ParaWin->EditFloat("*Hue"         ,38, 1, 0.0f, -4.0f, 4.0f, 0.001f);
    app->ParaWin->EditFloat("*Saturation"  ,39, 1, 1.0f, -4.0f, 4.0f, 0.001f);
    app->ParaWin->EditFloat("*Contrast"    ,40, 1, 1.0f, -64, 64, 0.001f);
    app->ParaWin->EditFloat("*Brightness"  ,41, 1, 0.0f, -64, 64, 0.001f);
  }
  ...
  if(op->SetDefaults)   
  {
    ...
    *op->Op.GetEditPtrF(39) = 1.0f;
    *op->Op.GetEditPtrF(40) = 1.0f;
  }
}
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditInt   ("sections"     , 0,1,    8,    3,  255,0.25f);
  app->ParaWin->EditInt   ("Slices"       , 1,1,    4,    3,  255,0.25f);
  if(!(op->Op.GetEditPtrU(6)[0]&1))
  {
    app->ParaWin->EditFloat ("Loop radius"    , 2,1, 0.5f,  -64,   64,0.002f);
    app->ParaWin->EditFloat ("Section radius" , 3,1, 0.125f,-64,   64,0.002f);
  }
  else
  {
    app->ParaWin->EditFloat ("Outer radius"  , 2,1, 0.5f,  -64,   64,0.002f);
    app->ParaWin->EditFloat ("Inner radius"  , 3,1, 0.125f,-64,   64,0.002f);
  }
  app->ParaWin->EditFloat ("Phase"        , 4,1, 0.5f, -256,  256,0.05f);
  app->ParaWin->EditFloat ("Arclen"       , 5,1, 1.0f,  -64,   64,0.002f);
  app->ParaWin->EditFlags ("!Radius mode"  , 6,1,"relative|absolute");
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("Filename"     , 0,"XSI File:xsi");
}

void Edit_MinMesh_OBJ(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFileName("Filename"     , 0,"OBJ File:obj");
}
...
void Edit_MinMesh_TransformEx(WerkkzeugApp *app,WerkOp *op)
{
  ...
  if(!app->TextureMode)
  {
    ...
    app->ParaWin->EditFlags ("Operation"    , 0,0,"*8none|normalize|polar");
  }
  else
  {
    app->ParaWin->EditFlags ("Flags"        , 0,0,"*2pos->|uv0->:*5->pos|->uv0");
  }
  app->ParaWin->EditFloat ("Scale"        , 1,3, 1.0f,-4096, 4096,0.01f);
  app->ParaWin->EditFloat ("Rotate"       , 4,3, 0.0f,  -64,   64,0.002f);
  app->ParaWin->EditFloat ("Translate"    , 7,3, 0.0f,-4096, 4096,0.001f);
...
void Edit_MinMesh_BoneTrain(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFlags ("Flags"       , 1,0,"shift|scale:*2|backwards");
  app->ParaWin->EditFloat ("Offset"      , 2,1, 0.0f,-64,64,0.001f);
...
void Edit_MinMesh_Font3D(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditString("Text"           , 0);
  app->ParaWin->EditString("Font"           , 1);
  app->ParaWin->EditFloat ("Height"         , 0,1,0.5f,0.0f,256.0f,0.001f);
  app->ParaWin->EditFloat ("Extrude"        , 1,1,0.1f,0.0f,256.0f,0.001f);
  app->ParaWin->EditFloat ("Max Error"      , 2,1,0.05f,0.0f,1.0f,0.001f);

  if(op->SetDefaults)
  {
    op->Op.SetString(0,"hund.");
    op->Op.SetString(1,"georgia");
  }
}
...
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags("Flags",0,0,"|scale tex:*1|add rings:*2|mul obj:*3|map continuous");
  app->ParaWin->EditFlags("Flags",0,0,"*4|map curves");
  app->ParaWin->EditFloat ("Scale Texture" , 1,1, 1.0f,-256, 256,0.001f);
  app->ParaWin->EditFloat ("Ring Distance" , 2,1, 1.0f,0, 4096,0.001f);
  app->ParaWin->EditFloat ("Mul Obj Distance" , 3,1, 1.0f,0, 4096,0.001f);
}
...
void Edit_MinMesh_Explode(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFlags ("!Flags"       ,15,0,"*0|Towards:*1|triangulate:*2|Tensor");
  app->ParaWin->EditFloat ("Center/Dir"   ,0,3,0.0f,-4096.0f,4096.0f,0.001f);
  app->ParaWin->EditSetGray(!(*op->Op.GetEditPtrU(15)&1));
  app->ParaWin->EditFloat ("Towards"     ,16,3,0.0f,-4096.0f,4096.0f,0.001f);
  app->ParaWin->EditSetGray(0);
  app->ParaWin->EditFloat ("Extrude Depth",3,1,0.0f,0.0f,256.0f,0.01f);
  app->ParaWin->EditFloat ("Extrude Angle",19,1,0.0f,-16.0f,16.0f,0.01f);
  app->ParaWin->EditSetGray(!(*op->Op.GetEditPtrU(15)&2));
  app->ParaWin->EditFloat ("Rotation",      8,3,0.0f,-64.0f,64.0f,0.001f);
  app->ParaWin->EditSetGray((*op->Op.GetEditPtrU(15)&2));
  app->ParaWin->EditFloat ("Tensor Rotation", 20,1,0.0f,-64.0f,64.0f,0.001f);
  app->ParaWin->EditSetGray(0);
  app->ParaWin->EditGroup ("Speed from");
  app->ParaWin->EditFloat ("Normal"     ,  4,1,0.0f,-4096.0f,4096.0f,0.01f);
  app->ParaWin->EditFloat ("Center",       5,1,0.0f,-4096.0f,4096.0f,0.01f);
  app->ParaWin->EditFloat ("Gravity",      6,1,0.0f,-4096.0f,4096.0f,0.01f);
  app->ParaWin->EditFloat ("Random",       7,1,0.0f,-4096.0f,4096.0f,0.01f);
  app->ParaWin->EditGroup ("Time Offset");
  app->ParaWin->EditFloat ("Time"  ,      14,1,0.0f,-64.0f,64.0f,0.0001f);
  app->ParaWin->EditFloat ("Distance",    11,1,0.0f,-64.0f,64.0f,0.0001f);
  if((*op->Op.GetEditPtrU(15)&1))
    app->ParaWin->EditFloat ("Radius",     12,1,0.0f,0.0f,64.0f,0.0001f);
  else
    app->ParaWin->EditFloat ("Dist Power",  12,1,0.0f,0.0f,64.0f,0.0001f);
  app->ParaWin->EditFloat ("Random"  ,    13,1,0.0f,-64.0f,64.0f,0.0001f);
}
...
void Edit_Scene_Select(WerkkzeugApp *app,WerkOp *op)
{
  app->ParaWin->EditOp(op);
  app->ParaWin->EditFloat("*Select"      , 0,1, 0.0f,0, 255,1.0f);
}
...
{
  app->ParaWin->EditOp(op);
  if(*op->Op.GetEditPtrU(6) & 4) // directional?
    app->ParaWin->EditFloat ("Rotate"       , 0,3, 0.0f,  -64,   64,0.002f);

  app->ParaWin->EditFloat ("*3Translate"   , 3,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditFlags ("!Flags"      , 6,2,"Ordinary|Weapon:*1|shadows:*2point|directional");
...
void Edit_Scene_Particles(WerkkzeugApp *app,WerkOp *op)
{
  ...
  app->ParaWin->EditFlags ("!"            , 0,0,"*6|come in:*7|don't loop");
  ...
  app->ParaWin->EditFloat ("Come out"    ,23,1, 0.0f,0,1,0.01f);
  app->ParaWin->EditFloat ("*Animate"     ,12,1, 0.0f,-4096, 4096,0.01f);
  app->ParaWin->EditFloat ("Rand Pos"    , 3,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditFloat ("Rand Speed"  ,16,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditFloat ("*Gravity"     ,19,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditSetGray((*op->Op.GetEditPtrU(0)&16)==0);
  app->ParaWin->EditFloat ("Rand Rotate", 6,3, 0.0f,  -64,   64,0.002f);
  app->ParaWin->EditFloat ("Rotate Speed",9,3, 0.0f,  -64,   64,0.002f);
  app->ParaWin->EditFloat ("Rand Rotate Speed" ,24,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditSetGray((*op->Op.GetEditPtrU(0)&1)!=0);
  app->ParaWin->EditFloat ("3Line"       ,13,3, 0.0f,-4096, 4096,0.005f);
  app->ParaWin->EditSetGray((*op->Op.GetEditPtrU(0)&1)==0);
  app->ParaWin->EditSpline("Spline"       , 0);
  app->ParaWin->EditSetGray(0);
  app->ParaWin->EditFloat ("Rand Forw Speed",22,1,0,-64,64,0.002f);
  app->ParaWin->EditGroup ("Cube-Mode");
  app->ParaWin->EditFlags ("Mode   ",0,0,"*8|Instances");
  app->ParaWin->EditGroup ("Pulsating");
  app->ParaWin->EditFloat ("*Moving wave"    ,27,1, 0.0f,-4096, 4096,0.01f);
  app->ParaWin->EditFloat ("*Standing wave"  ,28,1, 0.0f,-4096, 4096,0.01f);
  app->ParaWin->EditFloat ("*Amplitude"      ,29,1, 0.0f,0,64,0.01f);
}
...
/****************************************************************************/
  ...
  {
    "Load"      ,0x01,KC_ANY,0,0,0x06001000,COL_ALL|COL_WZT,'l',
    "",
    Edit_Misc_Load,
    Init_Misc_Load,
    Exec_Misc_Load,
  ...
  {
    "Store"     ,0x02,KC_ANY,1,1,0x26000100,COL_ALL|COL_WZT,'s',
    "",
    Edit_Misc_Store,
    Init_Misc_Nop,
    Exec_Misc_Nop,
  ...
  {
    "Nop"       ,0x03,KC_ANY,1,1,0x26000100,COL_ALL|COL_WZT,0,
    "",
    Edit_Misc_Nop,
    Init_Misc_Nop,
    Exec_Misc_Nop,
  ...
  {
    "EventTime"   ,0x16,KC_ANY,1,1,0x00000100,COL_ALL,0,
    "",
    Edit_Misc_EventTime,
    Init_Misc_EventTime,
    Exec_Misc_EventTime,
...
/****************************************************************************/
  ...
  {
    "ShakeEvent"    ,0x19,KC_SPLINE,1,2,0x40000211,COL_ADD,0,
    "bffffffffffffffff",
    Edit_Misc_Shaker,
    Init_Misc_Shaker,
    Exec_Misc_Nop,
  ...
  {
    "SplineScale"   ,0x1c,KC_SPLINE,1,1,0x40000104,COL_ADD,0,
    "fffb",
    Edit_Misc_SplineScale,
    Init_Misc_SplineScale,
    Exec_Misc_Nop,
    { KC_SPLINE },
    { 0 },
  },
...
/****************************************************************************/
  ...
  {
    "Flat"      ,0x21,KC_BITMAP,0,0,0x06000003,COL_NEW|COL_WZT,'f',
    "bbc",
    Edit_Bitmap_Flat,
    Bitmap_Flat,
    Exec_Misc_Nop,
  ...
  {
    "Perlin"    ,0x22,KC_BITMAP,0,0,0x0600000b,COL_NEW|COL_WZT,'p',
    "bbbbebbggcc",
    Edit_Bitmap_Perlin,
    Bitmap_Perlin,
    Exec_Misc_Nop,
  ...
  {
    "Color"     ,0x23,KC_BITMAP,1,1,0x06000102,COL_MOD|COL_WZT,'C'|sKEYQ_SHIFT,
    "bc",
    Edit_Bitmap_Color,
    Bitmap_Color,
    Exec_Misc_Nop,
  ...
  {
    "Merge"     ,0x24,KC_BITMAP,1,KK_MAXINPUT,0x07000001,COL_ADD|COL_WZT,'a',
    "b",
    Edit_Bitmap_Merge,
    Bitmap_Merge,
    Exec_Misc_Nop,
  ...
  {
    "Format"    ,0x25,KC_BITMAP,1,1,0x06000103,COL_XTR|COL_WZT,0,
    "bbb",
    Edit_Bitmap_Format,
    Bitmap_Format,
    Exec_Misc_Nop,
  ...
  {
    "Dots"      ,0x28,KC_BITMAP,1,1,0x06000104,COL_MOD|COL_WZT,0,
    "ccib",
    Edit_Bitmap_Dots,
    Bitmap_Dots,
    Exec_Misc_Nop,
  ...
  {
    "Blur"      ,0x29,KC_BITMAP,1,1,0x06000104,COL_MOD|COL_WZT,'b',
    "bggg",
    Edit_Bitmap_Blur,
    Bitmap_Blur,
    Exec_Misc_Nop,
  ...
  {
    "Mask"      ,0x2a,KC_BITMAP,3,3,0x06000301,COL_ADD|COL_WZT,0,
    "b",
    Edit_Bitmap_Mask,
    Bitmap_Mask,
    Exec_Misc_Nop,
  ...
  {
    "HSCB"      ,0x2b,KC_BITMAP,1,1,0x06000104,COL_MOD|COL_WZT,'h',
    //"eeef",
    "gggg",
    Edit_Bitmap_HSCB,
    Bitmap_HSCB,
  ...
  {
    "Transform" ,0x2c,KC_BITMAP,1,1,0x06000108,COL_MOD|COL_WZT,'r',
    "eggeebbb",
    Edit_Bitmap_Rotate,
    Bitmap_Rotate,
    Exec_Misc_Nop,
  ...
  {
    "Distort"   ,0x2d,KC_BITMAP,2,2,0x06000202,COL_ADD|COL_WZT,'d',
    "eb",
    Edit_Bitmap_Distort,
    Bitmap_Distort,
    Exec_Misc_Nop,
  ...
  {
    "Normals"   ,0x2e,KC_BITMAP,1,1,0x06000102,COL_XTR|COL_WZT,'n',
    "gb",
    Edit_Bitmap_Normals,
    Bitmap_Normals,
    Exec_Misc_Nop,
  ...
  {
    "Light"     ,0x2f,KC_BITMAP,1,1,0x0600010b,COL_MOD|COL_WZT,'L'|sKEYQ_SHIFT,
    "bgggeeccege",
    Edit_Bitmap_Light,
    Bitmap_Light,
    Exec_Misc_Nop,
  ...
  {
    "Bump"      ,0x30,KC_BITMAP,2,2,0x0600020e,COL_MOD|COL_WZT,'B'|sKEYQ_SHIFT,
    "bgggeeccegecge",
    Edit_Bitmap_Bump,
    Bitmap_Bump,
    Exec_Misc_Nop,
  ...
  {
    "Text"      ,0x31,KC_BITMAP,1,1,0x468a0109,COL_MOD|COL_WZT,'t',
    "eeggcbbgg",
    Edit_Bitmap_Text,
    Bitmap_Text,
    Exec_Misc_Nop,
  ...
  {
    "Cell"      ,0x32,KC_BITMAP,0,0,0x0600000d,COL_NEW|COL_WZT,0,
    "bbcccbbggbebf",
    Edit_Bitmap_Cell,
    Bitmap_Cell,
    Exec_Misc_Nop,
  ...
  {
    "Gradient"  ,0x34,KC_BITMAP,0,0,0x06000008,COL_NEW|COL_WZT,'G'|sKEYQ_SHIFT,
    "bbccgegb",
  ...
  {
    "Range"         ,0x35,KC_BITMAP,1,1,0x06000103,COL_MOD|COL_WZT,0,
    "bcc",
    Edit_Bitmap_Range,
    Bitmap_Range,
    Exec_Misc_Nop,
  ...
  {
    "TransformMul"  ,0x36,KC_BITMAP,1,1,0x0600010a,COL_MOD|COL_WZT,sKEYQ_SHIFT|'R',
    "eggeebcbbc",
  ...
  {
    "Twirl"         ,0x37,KC_BITMAP,1,1,0x06000107,COL_XTR|COL_WZT,0,
    "ggeeeeb",
    Edit_Bitmap_Twirl,
    Bitmap_Twirl,
    Exec_Misc_Nop,
  ...
  {
    "Sharpen"       ,0x38,KC_BITMAP,1,1,0x06000104,COL_MOD|COL_WZT,0,
    "beee",
    Edit_Bitmap_Sharpen,
    Bitmap_Sharpen,
    Exec_Misc_Nop,
  ...
  {
    "GlowRect"      ,0x39,KC_BITMAP,1,1,0x0600010b,COL_MOD|COL_WZT,'g',
    "eeeeeecegbb",
  ...
  {
    "Import"        ,0x3a,KC_BITMAP,0,0,0x46090000,COL_NEW|COL_WZT,0,
    "",
    Edit_Bitmap_Import,
    Bitmap_Import,
    Exec_Misc_Nop,
  ...
  {
    "ColorBalance"  ,0x3b,KC_BITMAP,1,1,0x06000109,COL_MOD|COL_WZT,'c',
    "eeeeeeeee",
  ...
  {
    "Unwrap"        ,0x3c,KC_BITMAP,1,1,0x06000101,COL_XTR|COL_WZT,0,
    "b",
    Edit_Bitmap_Unwrap,
    Bitmap_Unwrap,
    Exec_Misc_Nop,
  ...
  {
    "Bricks"        ,0x3d,KC_BITMAP,0,0,0x0600000e,COL_NEW|COL_WZT,0,
    "bbcccff" "bbbbbff",
    Edit_Bitmap_Bricks,
    Bitmap_Bricks,
    Exec_Misc_Nop,
  ...
  {
    "Bulge"         ,0x3e,KC_BITMAP,1,1,0x06000101,COL_XTR|COL_WZT,0,
    "g",
    Edit_Bitmap_Bulge,
    Bitmap_Bulge,
    Exec_Misc_Nop,
  ...
  {
    "Export"        ,0x41,KC_BITMAP,1,1,0x06010101,COL_ALL|COL_WZT,0,
    "",
    Edit_Bitmap_Export,
    Bitmap_Export,
    Exec_Misc_Nop,
  ...
  {
    "GeloetTex"     ,0x190,KC_BITMAP,0,1,0x0600000b,COL_NEW,0,
    "bbbbbbbbggg",
    Edit_Bitmap_Geloet,
    Bitmap_Geloet,
    Exec_Misc_Nop,
  ...
  {
    "Import 2"      ,0x42,KC_BITMAP,0,0,0x46090001,COL_NEW,0,
    "b",
    Edit_Bitmap_Import2,
    Bitmap_Import2,
    Exec_Misc_Nop,
  ...
  {
    "Paste"         ,0x43,KC_BITMAP,2,2,0x06000208,COL_ADD|COL_WZT,0,
    "gggggggb",
    Edit_Bitmap_Paste,
    Bitmap_Paste,
    Exec_Misc_Nop,
  ...
  {
    "VectorPath"    ,0x44,KC_BITMAP,1,1,0x46090102,COL_MOD,0,
    "cb",
    Edit_Bitmap_VectorPath,
    Bitmap_VectorPath,
    Exec_Misc_Nop,
    { KC_BITMAP },
    { 0 },
...
*/
  {
    "PartEmitter"   ,0x63,KC_SCENE,0,1,0x00000114,COL_NEW,0,
    "bbsff" "ffffffffffffbGG",
  ...
  {
    "Geloet2D"    , 0x71,KC_EFFECT,0,0,0x0001200b,COL_NEW,0,
    "Fbbb" "bbbbb" "gg",
    Edit_Effect_Geloet2D,
    Init_Effect_Geloet2D,
    Exec_Effect_Geloet2D,
    { 0 },
    { KC_MATERIAL,KC_MATERIAL },
  },
  {
    "GeloetMesh"  , 0x72,KC_EFFECT,1,1,0x00002108,COL_NEW,0,
    "F" "bbbbb" "gg",
    Edit_Effect_GeloetMesh,
    Init_Effect_GeloetMesh,
    Exec_Effect_GeloetMesh,
    { KC_MINMESH },
    { KC_MATERIAL,KC_MATERIAL },
  },
  {
    "Water"         ,0x73,KC_EFFECT,0,1,0x00001106,COL_NEW,0,
    "fffffi",
    Edit_Effect_Water,
    Init_Effect_Water,
    Exec_Effect_Water,
  ...
  {
    "Glare"       , 0x74,KC_EFFECT,0,0,0x0000000b,COL_NEW,0,
    "bbcccffffff",
    Edit_Effect_Glare,
    Init_Effect_Glare,
    Exec_Effect_Glare,
  ...
  {
    "Color Correction"    , 0x75,KC_EFFECT,0,0,0x0000000b,COL_NEW,0,
    "bffccccbccf",
    Edit_Effect_ColorCorrection,
    Init_Effect_ColorCorrection,
    Exec_Effect_ColorCorrection,
    { 0 },
    { 0 },
  },
  {
    "Grid stuff"    , 0x76,KC_EFFECT,0,0,0x00001004,COL_NEW,0,
    "bbcc",
    Edit_Effect_GridStuff,
    Init_Effect_GridStuff,
    Exec_Effect_GridStuff,
    { 0 },
    { KC_MATERIAL },
  },
  {
    "Cubehorde"     , 0x77,KC_EFFECT,0,0,0x24000011,COL_XTR,0,
    "FFFCFFFFFffbbCFCC",
    Edit_Effect_Cubes,
    Init_Effect_Cubes,
    Exec_Effect_Cubes,
  ...
  {
    "Bounceworm"    , 0x78,KC_EFFECT,0,0,0x20000007,COL_XTR,0,
    "iffffii",
    Edit_Effect_BounceWorm,
    Init_Effect_BounceWorm,
    Exec_Effect_BounceWorm,
    { 0 },
    { 0 },
  },
  {
    "Wirbeln"    , 0x79,KC_EFFECT,0,0,0x2000000c,COL_XTR,0,
    "ffffff" "bbfffi",
    Edit_Effect_Wirbeln,
    Init_Effect_Wirbeln,
    Exec_Effect_Wirbeln,
  ...
  {
    "XSI2Cube"      , 0x7a,KC_EFFECT,0,0,0x24090001,COL_XTR,0,
    "b",
    Edit_Effect_XSI2Cube,
    Init_Effect_XSI2Cube,
    Exec_Effect_XSI2Cube,
    { 0 },
    { 0 },
  },
  {
    "Cubemorpher"   , 0x7b,KC_EFFECT,1,KK_MAXINPUT,0x27000000,COL_XTR,0,
    "",
    Edit_Effect_CubeMorpher,
    Init_Effect_CubeMorpher,
    Exec_Effect_CubeMorpher,
    { KC_ANY },
    { 0 },
  },
  {
    "ImageList"     , 0x7c,KC_IMGLIST,1,KK_MAXINPUT,0x23000002,COL_ADD,0,
    "ss",
    Edit_Misc_ImageList,
    Init_Misc_ImageList,
    Exec_Misc_Nop,
  ...
  {
    "ImageJukebox"  , 0x7d,KC_EFFECT,0,0,0x00001005,COL_NEW,0,
    "bsggg",
    Edit_Effect_ImageJukebox,
    Init_Effect_ImageJukebox,
    Exec_Effect_ImageJukebox,
    { 0 },
    { KC_IMGLIST },
  },
...
/****************************************************************************/
  ...
  {
    "Perlin Displace"  ,0x91,KC_MESH,1,1,0x4600010d,COL_XTR,'p',
    "bFFFEEEFFFFFF",
    Edit_Mesh_Perlin,
    Mesh_Perlin,
    Exec_Misc_Nop,
...
/****************************************************************************/
  ...
  {
    "Grid"          ,0x101,KC_MINMESH,0,0,0x06000003,COL_NEW|COL_WZT,'g',
    "bbb",
    Edit_MinMesh_Grid,
    MinMesh_Grid,
    Exec_Misc_Nop,
  ...
  {
    "Cube"          ,0x102,KC_MINMESH,0,0,0x0600000d,COL_NEW|COL_WZT,'q',
    "bbbbggggggfff",
    Edit_MinMesh_Cube,
    MinMesh_Cube,
    Exec_Misc_Nop,
  ...
  {
    "Torus"         ,0x103,KC_MINMESH,0,0,0x06000007,COL_NEW|COL_WZT,'o',
    "bbffeeb",
    Edit_MinMesh_Torus,
    MinMesh_Torus,
    Exec_Misc_Nop,
  ...
  {
    "Sphere"        ,0x104,KC_MINMESH,0,0,0x06000002,COL_NEW|COL_WZT,'h',
    "bb",
    Edit_MinMesh_Sphere,
    MinMesh_Sphere,
    Exec_Misc_Nop,
  ...
  {
    "Cylinder"      ,0x105,KC_MINMESH,0,0,0x06000005,COL_NEW|COL_WZT,'z',
    "bbbbb",
  ...
  {
    "XSI"           ,0x106,KC_MINMESH,0,1,0x46090000,COL_NEW,'X'|sKEYQ_SHIFT,
    "",
    Edit_MinMesh_XSI,
    MinMesh_XSI,
    Exec_Misc_Nop,
  ...
  {
    "OBJ"           ,0x108,KC_MINMESH,0,1,0x46090000,COL_NEW,0,
    "",
    Edit_MinMesh_OBJ,
    MinMesh_OBJ,
    Exec_Misc_Nop,
...
/****************************************************************************/
  ...
  {
    "Add"           ,0x111,KC_MINMESH,1,KK_MAXINPUT,0x07000000,COL_ADD|COL_WZT,'a',
    "",
    Edit_MinMesh_Add,
    MinMesh_Add,
    Exec_Misc_Nop,
  ...
  {
    "Invert"         ,0x115,KC_MINMESH,1,1,0x06000100,COL_MOD|COL_WZT,0,
    "b", 
    Edit_MinMesh_Invert,
    MinMesh_Invert,
    Exec_Misc_Nop,
  ...
  {
    "MatInput"       ,0x117,KC_MINMESH,2,2,0x06000202,COL_MOD|COL_WZT,'M'|sKEYQ_SHIFT,
    "bb",
...
/****************************************************************************/
  ...
  {
    "Transform"     ,0x120,KC_MINMESH,1,1,0x0600010a,COL_MOD|COL_WZT,'t',
    "bGGGGGGGGG",
  ...
  {
    "TransEx"       ,0x121,KC_MINMESH,1,1,0x0600010c,COL_MOD|COL_WZT,'T'|sKEYQ_SHIFT,
    "iGGGGGGFFFbb",
  ...
  {
    "Displace"      ,0x123,KC_MINMESH,2,2,0x06000204,COL_XTR|COL_WZT,'d',
    "bFFF",
  ...
  {
    "Perlin Displace" ,0x124,KC_MINMESH,1,1,0x0600010d,COL_XTR|COL_WZT,'p',
    "bFFFEEEFFFFFF",
    Edit_MinMesh_Perlin,
    MinMesh_Perlin,
    Exec_Misc_Nop,
  ...
  {
    "BoneTrain"  ,0x129,KC_MINMESH,2,3,0x06000203,COL_XTR,0,
    "fbf",
  ...
  {
    "Pipe",0x12c,KC_MINMESH,2,4,0x06000404,COL_MOD,0,
    "bfff",
    Edit_MinMesh_Pipe,
    MinMesh_Pipe,
    Exec_Misc_Nop,
  ...
  {
    "Multiply"          ,0x12f,KC_MINMESH,1,1,0x46000111,COL_ADD|COL_WZT,0,
    "GGGGGGFFFbbGGGGGGF",
  ...
  {
    "Multiply 2"        ,0x130,KC_MINMESH,1,KK_MAXINPUT,0x07000014,COL_ADD|COL_WZT,0,
    "bbbbgggbbbgggbbbbggg",
  ...
  {
    "Center"            ,0x131,KC_MINMESH,1,1,0x06000101,COL_MOD|COL_WZT,0,
    "b",
    Edit_MinMesh_Center,
    MinMesh_Center,
    Exec_Misc_Nop,
  ...
  {
    "Font3D"            ,0x133,KC_MINMESH,0,1,0x06020003,COL_NEW,0,
    "ggg",
    Edit_MinMesh_Font3D,
    MinMesh_Font3D,
    Exec_Misc_Nop,
  ...
  {
    "Explode"      ,0x134,KC_MINMESH,1,2,0x06000215,COL_MOD,0,
    "fffffffffffffffbfffff",
    Edit_MinMesh_Explode,
    MinMesh_Explode,
    Exec_Misc_Nop,
    { KC_MINMESH,KC_BITMAP },
    { 0 },
...
/*{
  ...
  {
    "Particles"      ,0xc5,KC_SCENE,1,2,0x0010021e,COL_MOD,0,   // c5 was MapLight!
    "sibgggggggggf"
    "gggffffffffffffff",
  ...
  {
    "Select"        ,0x185,KC_SCENE,1,15,0x05000001,COL_ADD,0,
    "f",
    Edit_Scene_Select,
    Init_Scene_Select,
    Exec_Scene_Select,
  ...
  {
    "Material"     ,0xd0,KC_MATERIAL,0,1,0x2e007140,COL_MOD,'M'|sKEYQ_SHIFT,
  ...
  {
    "Viewport"      ,0xf0,KC_IPP,1,2,0x00000218,COL_NEW,'v',
  ...
  {
    "Blur"          ,0xe3,KC_IPP,1,1,0x00000105,COL_MOD,'b',
    "bfebb",
    Edit_IPP_Blur,
    Init_IPP_Blur,
    Exec_IPP_Blur,
  ...
  {
    "Copy"          ,0xe2,KC_IPP,1,1,0x00000103,COL_MOD,'C'|sKEYQ_SHIFT,
    "bcf",
    Edit_IPP_Copy,
    Init_IPP_Copy,
    Exec_IPP_Copy,
  ...
  {
    "Crashzoom"     ,0xe4,KC_IPP,1,1,0x00000106,COL_MOD,'z',
    "bbgeee",
    Edit_IPP_Crashzoom,
    Init_IPP_Crashzoom,
    Exec_IPP_Crashzoom,
  ...
  {
    "Sharpen"       ,0xe5,KC_IPP,1,1,0x00000104,COL_MOD,0,
    "becb",
    Edit_IPP_Sharpen,
    Init_IPP_Sharpen,
    Exec_IPP_Sharpen,
  ...
  {
    "Color"         ,0xe6,KC_IPP,1,1,0x00000104,COL_ADD,'c',
    "bcbc",
    Edit_IPP_Color,
    Init_IPP_Color,
    Exec_IPP_Color,
  ...
  {
    "Merge"         ,0xe7,KC_IPP,2,2,0x00000204,COL_ADD,'m',
    "bbbc",
    Edit_IPP_Merge,
    Init_IPP_Merge,
    Exec_IPP_Merge,
  ...
  {
    "Mask"          ,0xe8,KC_IPP,3,3,0x00000301,COL_XTR,'M'|sKEYQ_SHIFT,
    "b",
    Edit_IPP_Mask,
    Init_IPP_Mask,
    Exec_IPP_Mask,
  ...
  {
    "Layer2D"       ,0xe9,KC_IPP,0,1,0x0000110b,COL_MOD,'L'|sKEYQ_SHIFT,
    "bfffffffffb",
    Edit_IPP_Layer2D,
    Init_IPP_Layer2D,
    Exec_IPP_Layer2D,
  ...
  {
    "HSCB"          ,0xec,KC_IPP,1,1,0x00000105,COL_ADD,0,
    "bgggg",
    Edit_IPP_HSCB,
    Init_IPP_HSCB,
    Exec_IPP_HSCB,
  ...
  {
    "JPEG"          ,0xed,KC_IPP,1,1,0x00000103,COL_ADD,0,
    "bbg",
    Edit_IPP_JPEG,
    Init_IPP_JPEG,
    Exec_IPP_JPEG,
